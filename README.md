...
- [~] **LAN 每主机监控 + 设备专用规则** (随 WebUI 优化做) —— ① **设备规则已生效**: `source_ip_cidr` 现对透明 TCP + SOCKS-UDP + 透明 UDP 全路径匹配 (给 proxy_tcp_target 填 `source_ip` = 发起方 IP, v4-mapped 归一; 见 brain `source-ip-routing`)。DNS 查询维度暂 None (每主机 DNS 策略未实现)。② **每主机用量**: 用户态版**已做** (v0.10.0 Admin/Devices 设备表 = per-源 IP 连接数/字节/活跃, `/api/devices`)。剩 **eBPF 精确计数** (tc 看得到含 splice 直连全部流量, 用户态计数会漏直连) + 可选设备别名
