...
- [ ] **brutal group_id (吸收 tcp-brutal 2.0, P2)** —— 现状每条暖池/UDP-mux 连接各设 `brutal_rate`, 并发时聚合 = N×rate 超发 (brutal 无视丢包狂发, 假设"率=瓶颈带宽", N 条各全速淹链路)。tcp-brutal 2.0 的 groups 让同 `group_id` 连接**共享一个总速率** (谁发谁用), 修 N× 超发让 `brutal_rate` 回归"客户端聚合总量"语义。实现: `TCP_BRUTAL_PARAMS` 12B→16B (+group_id u64, 内核仍收 12B v1) + `getsockopt(TCP_BRUTAL_VERSION=23302)≥0x020000` 探测 (老模块回落 v1 per-socket); 客户端按 server_addr 分 group, 服务端按 client 分 group。⚠️需两端部署 v2 内核模块才生效。rules/brutalctl/`ip route proto 233` 不吸收 (透明路由用, Mirage 是 userspace per-socket 模型)
