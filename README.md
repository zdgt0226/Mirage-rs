...
![Mirage-rs](https://img.shields.io/badge/Language-Rust-f74c00.svg) ![Platform](https://img.shields.io/badge/Platform-Linux-blue.svg) ![Version](https://img.shields.io/badge/Version-v0.10.1-10b981.svg)
...
- [~] **UDP mux → QUIC Datagram** —— TCP-mux 已解带机量 (v0.9.0); QUIC 版解跨流队头阻塞 + 实时质量, 大工程 (吸收 queqiao, 见 `docs/quic-transport-design.md`)。**P0 骨架已做** (未发版, `--features quic` 默认关): 运行时开关 `transport: "quic"` 两端同设, QUIC 做底层字节管道、上面照跑 Mirage fake-TLS+AEAD (Model Y), 与 TCP 主链路并存。⚠️ **P0 不隐蔽** (quinn 默认指纹裸奔), 勿用于敌对网络; 指纹仿真 (P1) + erasure CC/FEC (P3) 后续。release 二进制暂不含
