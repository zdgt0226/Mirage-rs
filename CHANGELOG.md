...
### feat(shadowsocks): 上游出口支持 `2022-blake3-aes-128-gcm` / `2022-blake3-aes-256-gcm`
...
>
> 📌 仅实现 **AES 两种**(用户所需)。`2022-blake3-chacha20-poly1305` 未实现, 会明确报错。
> UDP 同样未实现(沿用 `upstream.udp` 默认 `block`)。
