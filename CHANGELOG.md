...
避免同一项目两套写法:
```json
"exclude": { "domain": ["router.lan"], "domain_suffix": ["apple.com","*.lan"],
             "domain_keyword": ["ads"], "domain_regex": [".*\\.cn$"] }
```
- `domain` 精确整域 (sing-box 风格, 非少见的 `domain_full`) · `domain_suffix` 根域+子域 (兼容 `*.`/`.`)
  · `domain_keyword` 子串 · `domain_regex` 正则整串 (忽略大小写, 非法项跳过 WARN)。大小写不敏感。
- 抽出可复用 `dns::domain_match::DomainMatcher` (`from_ruleset`/`from_parts`); `FakeIpMapper` 委托它。
  复用已依赖的 `regex` crate; 空规则零开销。
- 6 匹配器单测 + fake_ip 单测 + `mirage check` 结构化校验通过。
...
### feat(crypto): 填充方案 config 可换 + 热重载 (B')

新增 `tuning.tls_padding_scheme` (格式 `lo-hi;lo-hi;...`, 每项对应握手后第 i 条记录目标大小)。默认方案
被指纹化时, **改 config + 热重载即可换填充特征、无需重编/重启**; 两端各自从 config 读, 发端-only 生效、
**零协议改动**。非法/越界整串忽略并回落内置默认 (仅 WARN)。
- `PAD_SCHEME` 从编译常量提为 `cipher::DEFAULT_PAD_SCHEME` + `ArcSwap` 全局; `CryptoWriter::new` 快照,
  换方案对新连接生效 (池子持续换连接)。
- 未采用运营侧极少需要的"服务端协议热推 (B1)": 会加控制帧 + 版本矩阵 + desync 面, 对两端自控的
  部署模型不划算。真到大规模不可控客户端再议。
- 新增 3 单测 (解析合法/拒非法/空回落默认); crypto 全 46 测通过。
...
复审上条重组逻辑, 补三处小纰漏:
- **`want<12` 守卫**: 原 `want<2` 太松, DNS 报文至少 12B 报头, `want` 落 2..=11 会返回残缺"响应"。收紧 `want<12`。
- **总时限**: 原逐帧各 `TCP_TIMEOUT`(5s), 最坏 5s×64 帧≈320s, 持有效凭据对端可拖住任务。加 10s
  整体 `deadline`, 每帧等待取 `min(剩余, TCP_TIMEOUT)`。
- **抽纯函数可测**: 重组判定抽 `reassemble_tcp_dns(acc)->Reassembled{Need/Done/Malformed}`, +单测覆盖
  前缀单独成帧 / 跨帧大响应 / len 非法 / 多余尾字节 (原逻辑只有代码审查兜底)。
...
**统一出站接口 `OutboundNode::connect`** 走路由出 (SS 入 → router → 任意出站)。配合 routing
的 `inbound` 条件即可 "SS 入 X → 出站 Y" 编排。**connect() 在数据面的首个消费者**。
- 复用现有 SS crypto 反向: 新增 `server_handshake` (SIP004: 发下行 salt + SsWriter, 懒读客户端
...
用已下载的 `geoip.dat` 离线判节点区域 (`server` IP 所在国), 辅助负载均衡/选路"同区域分组":

- `mirage-rs test` 每节点显示 `[国家码]` (如 `[US]`/`[CN]`)。
- `import --group` 建组时组内节点跨多区域 → 告警 (出口国不一致影响落地解锁/延迟)。
- 新增 `router::geo::load_all_geoip` (解析全部国家) + `country_for_ip` (IP→国家反查)。
  CLI 从 `tuning.geodata_dir` 定位 geoip.dat; 无 geoip / 未知 IP 静默降级 (不报区域)。
...
- 全部通过退出 0, 有失败退出 1 (可做脚本闸门)。`--timeout` 可调 (握手阶段另有 15s 下限, 不低于
  协议自身对慢链路的握手容忍, 避免误判慢但可用的服务端)。
...
- **每上游 `protocol` (udp 默认 / tcp)**: UDP/53 被封或投毒的网络可给 `direct`/`cn` 上游加
  `"protocol": "tcp"` 走 TCP 解析 (RFC 7766 长度分帧, **并行竞速** + 3s 整体超时)。UDP 上游仍走
...
⚠️ **含一个真 P1 (建议升级)**: DNS 域名哈希碰撞 —— `foo.bar.com` 与 `foobar.com` 哈希相同,
DNS 缓存串扰会把流量劫持到错 IP 且极难排查。

外部审计四条核实结论: 1 真 P1 (DNS 碰撞, 已修) + 2 竞态真但 severity 夸大 (cgroup 自连 /
LPM 非原子, 均已修) + 1 已知取舍 (IPv6 硬编码, 归 roadmap)。规律一贯: 分析方向准, 严重性
系统性夸高, 建议不能照搬 (LPM 的双缓冲方案在 aya 上不划算, 改用先加后删)。
### LPM 同步先加后删 (外部审计 #3)
...
⚠️ **本版修复了两类"静默走错出口"的问题, 建议升级**:
- **SOCKS5 UDP 完全绕过路由规则** —— 默认直连时, 本该走隧道的 UDP 从本机 IP 裸奔出去;
  写了 `block` 的目标照发不误
- **`inbound` 规则对 transparent 的 UDP 不生效** —— 同一站点 TCP/UDP 两个出口 IP

WireGuard 已对**真实 WG 服务端五层验证互通**(握手 / 裸 IP 包 / TCP / UDP / 隧道内 DNS)。

### SOCKS5 UDP 逐数据报路由 (修复完全绕过路由规则)

### fix(udp): SOCKS5 UDP ASSOCIATE 此前**完全绕过路由规则**

出站在 **ASSOCIATE 那一刻**就用 `default_outbound` 定死 —— 而那时**还没有任何数据报**,
目标是未知的。目标其实逐数据报写在各自的 SOCKS5 头里 (`[RSV][FRAG][ATYP][ADDR][PORT]`),
`parse_socks_udp_addr` 早就在解析它, 只是解析结果**从来没送进路由器**。

后果有方向性:
- 默认 `direct` + 规则说某域名走代理 → **UDP 从本机 IP 裸奔出去**(泄漏)
- 规则说某目标 `block` → **照发不误**(拦截失效, 用户以为拦住了)

改为**逐数据报路由**。关键简化: 会话按**出站 tag** 建而非按目标建 ——
- Mirage 隧道本身多路复用目标 (每帧自带地址, 服务端逐帧解析), 一条隧道服务所有目标;
- 直连 socket 可 `send_to` 任意目标, 一个 socket 够用。

因此不需要 `transparent_udp` 那套按目标的会话表与引用计数配平。同一个关联里不同数据报
**可以走不同出站**, 这正是修复的意义。`block` 改为**逐包丢弃**而非拒绝整个关联。

原来的两个实现 (`handle_udp_associate` 走隧道 / `handle_udp_associate_direct` 直连) 被本次
改动孤立, 已删除, 合并为一个 `handle_udp_associate_routed`。

> ⚠️ **第三次踩同一个假测试坑**: 回归测试初版把被 block 的目标设成
> `127.0.0.2:9` —— 那里**根本没人监听**, 所以"收不到回程"在完全没有路由的情况下也成立,
> 判据恒真。变异测试 (把出站改回 ASSOCIATE 时定死) 当场拆穿: 变异下测试照样通过。
> 已改成**两个都真在监听**的 echo, 按目标端口 block 其中一个 —— 这样"收不到回程"的
> 唯一解释就是路由拦了它。重新变异验证: 变异下必红。

### 自我审查修复 (含 Sonnet 独立复核)
...
多入站部署下"同一个域名, 从不同入站进来走不同出口"此前做不到。新增 `routing.rules[].inbound`
(单值或数组), 只对来自这些入站的连接生效; 不写 = 不限入站, 老配置行为不变。

入站 tag 从 socks / mixed / transparent 三处入站定义穿透到路由判定 (每连接一次 `Arc<str>`
克隆)。lite 模式无多入站概念, 显式传 None。

**信息缺失时不猜**: 调用方没提供入站 tag 时, 带 `inbound` 条件的规则**一律不命中**。
反过来 (缺失就放宽成"任意入站都算") 会让本该只对某入站生效的规则**外溢**到所有连接 ——
例如"家人那条入站走固定落地"的规则会把自己的流量也带过去, 而且完全无声。宁可不命中。
已变异验证。

`check` 校验覆盖: 引用不存在的入站 tag 会被拦下 —— 拼错入站名很常见, 而症状是
"规则写了不生效", 没有这条提示极难自查。

### geo 更新器: 借鉴 Python 版的两条血泪教训
...
### fix(geo): 超时拆成三段 —— 原来的单一 30s 总超时让大文件**永远下不完**

真机下载验证当场抓到的生产问题: 原实现只设 `.timeout(30s)`, 而 reqwest 的 `timeout` 是
**含 body 的整请求时长**。geo 文件十几 MB, 慢链路上 30 秒根本下不完, 于是每次都是
`read body: error decoding response body`, 更新**永远失败**, 而错误信息完全指不到
"其实是超时太短"。

改成三段, 各司其职:
- `connect_timeout` 15s —— 对端不可达时快速失败, 别耗掉整个预算
- `read_timeout` 60s —— **两次读之间**的最大静默。只要还在收数据就不打断, 对慢速大文件
  友好, 又能识别"连上了但卡死不发"
- `timeout` 600s —— 总预算兜底, 只防"一直慢慢挤字节"的 slowloris 式极端情况

600s 给得宽是**有实测依据的**: 同一个 geoip.dat (17.7 MB), 一次 559 KB/s 用 31s,
紧接着一次只有 48 KB/s 用了 360s —— 同机同链路差一个数量级。总预算给 300s 时后者直接失败。
代价可控: updater 在后台任务里, 单源拖久了不阻塞其他流程。

### 真机下载验证 (`geo_real_download_and_304`, `#[ignore]` 默认不跑)

单测只能证明"元数据能往返、校验判据对", **证明不了上游到底认不认条件请求**。这条真跑三段:

| 阶段 | 实测结果 |
|---|---|
| ① 冷下载 | 17,784,192 字节, 解析出 **260 个分类**, 拿到 `ETag` 与 `Last-Modified` |
| ② 立刻再跑 | 新鲜度命中, 文件未被重写, 未发请求 |
| ③ 拨老 downloaded_at | **上游回 304**, 本地文件未被重写 —— 省掉一次 17MB 全量下载 |

**GitHub release assets 确实支持条件请求** —— 这点此前只能推测, 现在证实。
跑法: `cargo test --lib geo_real_download -- --ignored --nocapture`

### feat(geo): 条件请求 (ETag / Last-Modified) + 多镜像 fallback

把 Python 版剩下的两条也补齐:

- **条件请求**: 新增 `meta.json` (原子写) 持久化每个源的 ETag / Last-Modified /
  `downloaded_at`。到期后先带 `If-None-Match` / `If-Modified-Since` 问一次, 上游回 304
  就复用本地文件, 省掉几 MB 传输。
  - 验证器**仅当本地 .dat 还在时**才带 —— 否则会出现"上游说你已是最新, 而本地文件早被
    删了"的尴尬, 结果是永远拿不到文件 (Python 版踩过同款并明确注释)。
  - 304 时虽然没重写文件, 也要把 `downloaded_at` 记一笔, 否则每个周期都会重新问一次。
    这也是新鲜度判据优先取 meta 而非文件 mtime 的原因 —— 304 不动 mtime。
  - meta 损坏/丢失一律退回空表: 它只是优化, 丢了最多多下一次, 不该让 updater 崩掉。
- **多镜像**: `geo_sources[].url` 现在既接受单个字符串, 也接受数组 (复用配置里已有的
  `one_or_many`)。逐个镜像试, 每个镜像内仍保留原有的 proxy→direct 通道回落。
  全部失败才算失败, 调用方据此保留旧文件。

### 裸 IP 目标也按域名分流 (SNI/Host 嗅探扩到全部入站)
...
### 阶段3a: pump 重构 (同步锁 + 绝不持锁 await)
`poll_read`/`poll_write` 是同步的, 锁不了 async mutex —— `TunnelInner` 改用 `std::sync::Mutex`。
随之而来的硬约束: 持锁期间**只做纯计算**, 把待发数据报收集起来在锁外发送。原先 `drain_tx`
在持锁时 `await udp.send`, 那样会把 runtime worker 连同锁一起卡住。顺带修掉审计里"持锁做
网络 IO"那条。

### 阶段3b: `WgTcpStream` (smoltcp 轮询 socket → tokio AsyncRead/AsyncWrite)
- 靠 smoltcp 的 `register_recv_waker`/`register_send_waker` (需开 `async` feature) 桥接,
  **不轮询不 sleep** —— 否则要么烧 CPU 要么加延迟。
- `poll_read` 严格区分"暂时没数据"(Pending) 与"对端已关闭"(EOF): 搞混要么永远挂着,
  要么把正常连接误判成断开。
- `send_slice` 后必须 `poll_now()`: 数据进发送缓冲 ≠ 发出去了, 要驱动 smoltcp 生成 IP 包
  并叫醒 pump 加密发送。漏了表现为"写成功但对端半天收不到"。
- `Drop` 里 `close()` + `sockets.remove()`: 不摘除则每条连接泄漏 128KB 缓冲。

> 测试纪律 (第二次踩同一个坑): `connect_drives_traffic_immediately` 又是**假测试** ——
> 握手进行中 boringtun 对后续 `encapsulate` 一律返回 Done, 5s 内不会有任何包出去, 用"对端
> 收没收到包"做判据恒真。已删除, 换成 `wake_drains_tx_promptly`: 直接观察 tx 队列是否被
> 及时排空, 10 次重复试验压掉 tick 干扰。变异验证 (去掉 `notify_one`) 由 10/10 掉到 **0/10**。

### 阶段3c-1: 客户端出站 `OutboundConfig::Wireguard` 接线
配置字段对齐 wg-quick (private_key / peer_public_key / preshared_key / endpoint / address /
mtu / persistent_keepalive), 可在 routing 里按规则选中。

- **隧道懒初始化**: 第一条路由到该出站的连接才真正建隧道 —— 否则没流量也白起一个 pump
  任务反复发握手。失败不缓存, 下一条连接可重试。
- **配错降级为 `block` 而非 `direct`**(安全契约): 用户配 WG 的意图就是流量从 WG 出去,
  悄悄改走直连 = 本该走隧道的流量从本机 IP 裸奔出去且毫无察觉。与 SS 上游的 UDP block
  同一条理由 —— 安全的失败方式是"不发"而不是"发到别处去"。已变异验证。
- **配置校验**(`check` 与启动共用): 密钥 base64 长度、address 是否合法 IP(不带掩码)、
  endpoint 是否带端口、mtu 范围。这些错配的共同特征是不会让进程起不来, 而是让**每条连接
  静默失败** —— 服务看着健康却什么都代理不了。
- 数据面用 `copy_bidirectional` 而非 Direct 的 splice(2): `WgTcpStream` 不是真 fd,
  数据在用户态 smoltcp 里, 无法零拷贝。

> ⚠️ 已知限制: **隧道内没有 DNS**。域名在本机解析后才把 IP 送进隧道, 即 DNS 查询不经 WG。
> 「只想换出口」够用; 若指望 WG 出站同时隐藏 DNS 或拿对端地区的 CDN 解析, 当前做不到。

### 本地 CI (`scripts/ci-local.sh`)
镜像 `build.yml`。动因: feature 分支**不触发任何 workflow**(只认 main 的 push 和指向 main
的 PR), 分支开发期本地跑是从 0 到 1。

⚠️ 本地绿 ≠ CI 绿, 两件事本地测不到: ①**内核版本** —— CI 故意跑 ubuntu-22.04(5.15)来检验
README 的 "≥5.10" 声明, 本地是 6.1; build.yml 里就记着实例(orphan 验证器本地绿、CI 红)。
...
### 阶段4: 对着真实 WireGuard 服务端的互通验证 ✅ 全部通过
`examples/verify_wg_real_peer.rs` —— 分层诊断验证器, 失败时一眼看出断在哪一层。
私钥从文件读 (不进源码、不进命令行历史), 其余参数走环境变量。

对一台真实 WG 服务端实测 (靶子 = 隧道内的 DNS 服务, TCP/UDP 同址):

| 层 | 内容 | 结果 |
|---|---|---|
| L1 | Noise IK 握手 (裸 boringtun) | ✅ 服务端回 type=2, 会话建立 |
| L1.5 | 裸 IP 包经隧道 (绕开 smoltcp) | ✅ 收到隧道内 DNS 响应 |
| L1.6 | ICMP echo → 服务端隧道地址 | ✅ 收到 echo reply |
| L2 | **TCP 经 smoltcp 栈** | ✅ DNS over TCP 往返 (105B, 含回答) |
| L3 | **UDP 经隧道** | ✅ DNS over UDP 往返 (105B, 含回答) |

**这消掉了此前"所有测试都是我方自洽、没证明能和真实 WireGuard 服务器互通"的边界。**
握手、密钥派生、报文格式、DATA 包双向加解密、smoltcp 产出的 TCP/UDP 包被真实服务端路由
并原路返回 —— 全链路已实测。另有一个强旁证: 开放端口连通、关闭端口 (53/443) 正确报 RST
拒绝, 说明 TCP 状态机的区分能力正常, 不是"瞎连都成功"。

> 分层的价值在过程中兑现了: 初测 L2/L3 失败, 但 L1.5 (绕开全部 smoltcp 代码的裸 IP 包)
...
### 隧道内 DNS (`dns` 字段)
新增 `wg::dns` 模块 + `WgConfig.dns`(对齐 wg-quick 的 `DNS =`)。配了就让走该隧道的域名
**经隧道解析**; 不配保持本机解析, 不强加额外往返。

动机与"出口 IP 不一致"是同一类错误, 只是发生在更早一步: 本机解析拿到的是**本机所在地区**
的结果, 对 CDN/流媒体意味着流量虽从对端出去、目标 IP 却是按你本地位置挑的 —— WG 出口白配。

DNS 报文解析是漏洞温床, 明确防住的几点(都有测试):
- **压缩指针环** —— 指向自己的指针会让朴素实现无限循环 (经典 DoS)。限制跳转次数。
- **响应 ID 校验** —— 不校验的话一个串进来的响应就能把域名解析到攻击者指定的 IP。
- **截断报文**、NXDOMAIN/SERVFAIL、**TTL 钳制**(上限防一条超长 TTL 把错误答案钉死)。

缓存每隧道一份(不与本机解析缓存混用 —— 两者答案含义不同), 条目封顶防唯一域名洪泛。
