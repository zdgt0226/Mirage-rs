...
EOM
    ...
    if [[ "$deploy_mode" == "2" ]]; then
        ...
        # 地址不带端口默认 53 (无需写 :53); 要走 TCP 解析在生成后手动给该 resolver 加 "protocol": "tcp"。
        direct_dns=$(ask "直连(国内)DNS 主用 (不带端口默认 53)" "223.5.5.5")
        direct_dns2=$(ask "直连(国内)DNS 备用 (多上游并行兜底, 留空=只用主用)" "114.114.114.114")
