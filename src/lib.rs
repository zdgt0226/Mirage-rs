            ...
            crate::config::InboundConfig::MirageServer { listen, port, password, camouflage_host, brutal_rate_mbps, auth_ts_tolerance_secs, upstream, pfs, transport, quic_window_mb, quic_erasure_cc, .. } => {
                ...
                match transport {
                    crate::config::Transport::Quic => {
                        #[cfg(feature = "quic")]
                        tokio::spawn(async move {
                            crate::proxy::mirage_server::start_quic_server(&listen_addr, &password, &cam_host, auth_ts_tolerance_secs, ss_upstream, pfs, quic_window_mb.unwrap_or(4), quic_erasure_cc.unwrap_or(true)).await;
                        });
                        #[cfg(not(feature = "quic"))]
